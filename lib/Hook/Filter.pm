...
---TODO!!!!!!!!
    ...
    # allow calls only if the subroutine's first argument matches /bob/
    args(1) =~ /bob/
...
Hook::Filter::Plugins::. See those modules for details.
...
=item * Time. Hook::Filter evaluates all filter rules for each call to a
...
Why the hell would one want such a creepy thing?
